...
- **Listing-statussen (Fase 27)** — `DRAFT | ACTIVE | PAUSED | RESERVED | PARTIALLY_SOLD | SOLD | DELETED`. DRAFT (concept opslaan zonder publiceren via `saveDraft`/`publishDraft`/`deleteDraft`), PAUSED (tijdelijk verbergen via `pauseListing`/`resumeListing` — automatische cascade rejecteert PENDING enkel-listing proposals én PENDING multi-listing bundle-offers met systeembericht, zelfde patroon als DELETED-cascade), RESERVED (gekoppeld aan een ACCEPTED bundle-offer of partial-balance proposal), PARTIALLY_SOLD (Fase 27.13 — MULTI_CARD partial-sale + stocked SEALED/OTHER waar nog AVAILABLE-rijen zijn). Account-limits cap is op `status="ACTIVE"` — DRAFT/PAUSED/PARTIALLY_SOLD tellen niet mee. Pickup-locatie via `pickupCity` (auto-fill uit User.city; alleen plaats publiek). `closePartiallySoldListing` (Fase 27.34) sluit een PARTIALLY_SOLD listing zonder buyer-koppeling: status → SOLD, items blijven historisch op AVAILABLE, cascade-rejects PENDING.
...
- **Direct Kopen voor non-stocked FIXED (Fase 27.33)** — `BuyNowButton` component op listing-detail page voor SINGLE_CARD/MULTI_CARD/COLLECTION met FIXED + allowDirectBuy + deliveryMethod !== "PICKUP". Hergebruikt `PaymentMethodModal`-patroon van BuyQuantityForm (single-quantity variant). PICKUP-only listings krijgen alleen `ContactSellerButton` (chat regelt EXTERNAL-reservering). NEGOTIABLE en FIXED+acceptsOffers krijgen "Bod doen"-knop (chat-route). Hint "verkoper accepteert alleen biedingen via chat" als Direct Kopen + biedingen beide uit staan. ContactSellerButton accepteert nu een optionele `label` + `variant` ("primary" | "outline") prop voor contextuele overrides.
...
| `listing.ts` | CRUD listings + `buyListing` (escrow) + `saveDraft`/`publishDraft`/`pauseListing`/`resumeListing`/`deleteDraft`/`closePartiallySoldListing` (Fase 27) + cascade naar PENDING proposals én bundle-offers |
...
| `listing/` | `listing-form`, `multi-step-listing-form` (DRAFT-knop + delivery-method-toggle + pickup-locatie velden + koop-toggles, Fase 27), `steps/` (incl. conditionRange UI voor COLLECTION + buy-options + suggested-price, Fase 27.31), `listing-card` (stock-badge "X op voorraad" voor SEALED/OTHER, Fase 27.36), `listing-actions` (status-aware: DRAFT/ACTIVE/PAUSED/RESERVED/PARTIALLY_SOLD knoppen incl. "Advertentie afsluiten", Fase 27.34), `listing-status-badge` (Fase 27), `buy-quantity-form` (stocked, Fase 27.23), `buy-now-button` (non-stocked FIXED Direct Kopen, Fase 27.33), `sponsored-row` |
...
User-velden toegevoegd in roadmap-werk: `maxRunnerUpAttempts` (Cluster A), `iban`/`accountHolderName`/`lastIbanChange` (Fase 5), `suspendedUntil`/`suspensionType`/`suspensionReason`/`suspensionAdminId` (Fase 8). Auction-velden: `runnerUpEnabled`/`failedBidderIds`/`runnerUpAttempts` (Cluster A). Listing.expiresAt + `EXPIRED` enum-waarde **verwijderd** (Cluster C). User-relatie `adminAuditLogs` (Fase 15, named `AdminAuditLogActor`). Listing-velden `pickupPostalCode`/`pickupCity` + status-uitbreiding `DRAFT/PAUSED/RESERVED` + relations naar `BundleProposalListing[]`/`BundleListing?` (Fase 27). ShippingBundle-velden `paymentMode` (PLATFORM|EXTERNAL), `bundleProposalId @unique`, `pickupReservationExpiresAt` + status-uitbreiding `SCHEDULED` + relations naar `bundleListings BundleListing[]`/`pickupSchedule PickupSchedule?` (Fase 27). Message-velden `bundleProposalId`/`pickupScheduleId` (Fase 27). User-relations `bundleProposalsAsBuyer/Seller` + `pickupSchedulesProposed` (Fase 27).
...
`common`, `auth`, `home`, `auction`, `claimsale`, `wallet`, `dashboard`, `profile`, `chat`, `proposal`, `listing`, `watchlist`, `notifications`, `bids`, `search`, `reputation`, `carousel`, `breadcrumbs`, `seller`, `cart`, `shipping`, `sellerClaims`, `subscription`, `verification`, `sales`, `purchases`, `disputes`, `customization`, `footer`, `withdrawal` (Fase 6), `blockReport` (Fase 7), `suspension` (Fase 8), `cancellation` (Fase 9), `admin` (Fase 15 — bevat shieldTooltip/panelTitle/sectie-namen/nav-keys/KPI-labels/quick-actions; pagina-specifieke strings nog hardcoded NL als follow-up), `bundleOffer` + `pickup` (Fase 27 — chat-bubble labels, status-keys, payment-deadlines, errors). `listing` namespace uitgebreid met `status.*`, `sections.*`, `actions.*`, `deliveryShip/Pickup/Both`, `pickupLocation.*` (Fase 27).
...
- (Fase 27.31) Direct Kopen voor PICKUP-only listings is bewust niet geïmplementeerd. Een PICKUP-only listing met FIXED+allowDirectBuy zou een "Reserveer voor ophalen" knop kunnen krijgen die buyListing met paymentMode=EXTERNAL aanroept (geen escrow, listing → RESERVED + 14d cron-timeout). Dat vereist een eigen flow in `buyListing` of een aparte action `reserveForPickup`.
