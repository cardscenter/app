...
- **Listing-statussen (Fase 27)** — `DRAFT | ACTIVE | PAUSED | RESERVED | SOLD | DELETED`. DRAFT (concept opslaan zonder publiceren via `saveDraft`/`publishDraft`/`deleteDraft`), PAUSED (tijdelijk verbergen via `pauseListing`/`resumeListing` — automatische cascade rejecteert PENDING enkel-listing proposals én PENDING multi-listing bundle-offers met systeembericht, zelfde patroon als DELETED-cascade), RESERVED (gekoppeld aan een ACCEPTED bundle-offer of partial-balance proposal). Account-limits cap is op `status="ACTIVE"` — DRAFT/PAUSED tellen niet mee. Pickup-locatie velden (`pickupPostalCode`, `pickupCity`) verplicht bij `deliveryMethod ∈ {PICKUP, BOTH}`.
...
| `listing.ts` | CRUD listings + `buyListing` (escrow) + `saveDraft`/`publishDraft`/`pauseListing`/`resumeListing`/`deleteDraft` (Fase 27) + cascade naar PENDING proposals én bundle-offers |
...
| `listing/` | `listing-form`, `multi-step-listing-form` (DRAFT-knop + delivery-method-toggle + pickup-locatie velden, Fase 27), `steps/`, `listing-card`, `listing-actions` (status-aware: DRAFT/ACTIVE/PAUSED/RESERVED knoppen, Fase 27), `listing-status-badge` (Fase 27), `sponsored-row` |
...
| `pickup-config.ts` | (Fase 27) `PICKUP_CODE_LENGTH` (4), `PICKUP_CODE_MAX_ATTEMPTS` (5), `PICKUP_LOCKOUT_HOURS` (1), `PICKUP_REMINDER_HOURS` (24), `generatePickupCode()` server-helper |
