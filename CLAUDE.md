...
- (Fase 28) Refund-toggle op gegroepeerde items: `groupBundleItems` produceert één rij voor 5× dezelfde booster met één `id` (eerste van de groep). `SellerRefundForm` markeert dan slechts 1 item als refunded. Bestaand probleem dat in deze fase niet is opgelost — workaround is custom-amount refund. Echte fix: ofwel ungrouped items doorgeven aan SellerRefundForm, ofwel issueSellerRefund een quantity-param geven.
- (Fase 28) Refund alleen op SHIPPED-bundles. Post-COMPLETED (auto-confirm na 30 dagen) gaat via dispute-flow. Cardmarket laat refund tot lang na delivery toe — eigen business-keuze, niet aangepast in deze fase.
