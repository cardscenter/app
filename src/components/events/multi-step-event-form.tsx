    ...
    case 3:
      if (form.entryType === "PAID") {
        if (form.entryPriceMode === "TIERS") {
          const valid = form.ticketTypes.some((t) => t.name.trim() && Number(t.price) >= 0 && t.price !== "");
          if (!valid) return "Voeg minstens één ticket-soort met naam en prijs toe";
        } else if (!form.entryPrice || Number(form.entryPrice) <= 0) {
          return "Vul een entreeprijs in";
        }
      }
      if (form.registrationRequired && !form.registrationUrl.trim()) return "Vul de aanmeld-/ticketlink in";
      return null;
    case 6:
...
export function MultiStepEventForm({ accountType }: { accountType: string }) {
  ...
  function buildFormData(confirm: boolean): FormData {
    ...
    if (form.entryType === "PAID") {
      fd.set("entryPriceMode", form.entryPriceMode);
      fd.set("entryCurrency", form.entryCurrency);
      if (form.entryPriceMode === "TIERS") {
        const tiers = form.ticketTypes
          .filter((t) => t.name.trim() && t.price !== "")
          .map((t) => ({ name: t.name.trim(), price: Number(t.price) }));
        fd.set("ticketTypes", JSON.stringify(tiers));
      } else {
        fd.set("entryPrice", form.entryPrice);
      }
    }
    if (form.childrenFreeUntilAge) fd.set("childrenFreeUntilAge", form.childrenFreeUntilAge);

    if (form.vendorTablePrice) fd.set("vendorTablePrice", form.vendorTablePrice);
    if (form.vendorChairPrice) fd.set("vendorChairPrice", form.vendorChairPrice);
    fd.set("vendorPowerAvailable", form.vendorPowerAvailable ? "1" : "0");
  ...
  function publish(confirm: boolean) {
    // Veiligheidscheck: loop alle stappen langs voor verplichte velden.
    ...
    <div className="mx-auto max-w-3xl">
      ...
      {duplicates && (
        <div className="mb-5 rounded-xl border border-amber-300 bg-amber-50 p-4 dark:border-amber-700 dark:bg-amber-950/40">
          ...
          <p className="mt-2 text-sm text-amber-700 dark:text-amber-300">
            Is dit een ander evenement? Dan kun je hieronder toch publiceren.
          </p>
        </div>
      )}
      ...
      <div className="mt-5 flex items-center justify-between">
        ...
        {isLast ? (
          <button
            type="button"
            onClick={() => publish(confirmDuplicate)}
            disabled={isPending}
