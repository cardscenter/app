...
export function SocialShare({ title, url }: SocialShareProps) {
  ...
  const platforms = [
    ...
    },
    {
      ...
      color: "hover:bg-neutral-500/10 hover:text-neutral-900 dark:hover:text-neutral-100",
      icon: (
        <svg viewBox="0 0 24 24" fill="currentColor" className="size-4">
      ...
      <div className="flex items-center gap-1">
        ...
        {platforms.map((p) => (
          <a
            ...
            className={`rounded-lg p-1.5 text-muted-foreground transition-all ${p.color}`}
            title={t("shareOn", { platform: p.name })}
          >
            {p.icon}
          </a>
        ))}
